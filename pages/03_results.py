...
# -----------------------------------------------------------------------------
# Helper for Protenix CIF loading and 3D visualisation
# -----------------------------------------------------------------------------


def load_protenix_cif(output_root: Path, round_num, barcode):
    """Locate the first CIF file generated by Protenix for a given variant.
...
def render_protenix_cif(cif_path: Path):
    """Render a Protenix CIF file with py3Dmol highlighting ligand (chain B)."""
    try:
        ...
        if st.session_state.selected_view == "Summary":
            ...
            if "docking_score" not in df.columns or df["docking_score"].notna().sum() == 0:
                ...
                if docked_df.empty:
                    ...
                    # Protenix preview
                    cif_path = load_protenix_cif(output_dir_path, result["round"], result["barcode"])
                    if cif_path and cif_path.exists():
                        components.html(render_protenix_cif(cif_path), height=600, width=800)
