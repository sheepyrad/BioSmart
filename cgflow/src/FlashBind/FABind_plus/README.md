...
**Checkpoints are not vendored in this tree.** Use the CGFlow setup script (recommended) or clone upstream FABind with git-lfs:

```shell
# Recommended (from cgflow repo root)
./scripts/setup/download_flashbind_assets.sh --fabind-only

# Or upstream FABind clone
...
Place the pre-trained regression model at `ckpt/fabind_plus_best_ckpt.bin` and the sampling model at `ckpt/confidence_model.bin`. Download via `./scripts/setup/download_flashbind_assets.sh --fabind-only` or from [Hugging Face — KyGao/FABind_plus_model](https://huggingface.co/KyGao/FABind_plus_model).
