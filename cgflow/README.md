...
### FlashBind / FABind+ weights (optional)

FlashBind optimization uses scoring checkpoints and FABind+ docking weights that are **not** stored in git. After install, download from Hugging Face:

```bash
./scripts/setup/download_flashbind_assets.sh
```

See [scripts/setup/README.md](scripts/setup/README.md) for details. FABind+ inference also needs the separate `fabind` conda env (see `src/FlashBind/FABind_plus/README.md`).
